...
CallStackStatus BackTracer::GenCallStack(std::stack<UNWValue> &q, bool verbose /*=false*/) {
    std::queue<UNWValue> pyFrameQueue;
    CallStackStatus status;
    if (profilerConf->doPyUnwinding) pyBackTrace(pyFrameQueue);
    ...
    while (unw_step(&cursor) > 0) {
        ...
        if (profilerConf->doPyUnwinding && std::string(outer_name).find("_PyEval_EvalFrameDefault") != std::string::npos) {
...
BackTracer* GetBackTracer() {
    return BackTracer::GetBackTracerSingleton();
}
